...
function inputHorizontalPaddingClassName(
  ...
  if (hasLeftAdornment && hasRightAdornment) return 'pl-1 pr-1';
  if (hasLeftAdornment) return 'pl-1 pr-4';
  if (hasRightAdornment) return 'pl-4 pr-1';
  return 'px-4';
}
  ...
  return (
    <Controller
      ...
      render={({ field: { onChange, onBlur, value, ref }, fieldState: { error } }) => {
        ...
        const displayValue = mask ? mask.mask(canonical) : canonical;
        const handleChangeText = (text: string) =>
          onChange((mask ? mask.unmask(text) : text) as any);
